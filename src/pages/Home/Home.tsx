...
export const Home = () => {
    ...
    useEffect(() => {
        getPokemon()
    }, [page, handlePage, getPokemon, digimons]);
    ...
    return (
        <Flex vertical={true} align={"center"} gap={"16px"} style={{padding: "24px 0"}}>
                    ...
                    digimons?.map((digi: Digimon, i: number) => {
                        return (
                            <Card key={i} cover={<img src={`${digi.image}`} className={styles.Card}
                                    alt={""}/>} style={{width: "18%"}} onClick={() => navigate(`/details/${digi.id}`)}>
            ...
            <Pagination defaultCurrent={page} total={totalPages * 10} onChange={(n) => handlePage(n)}/>
        </Flex>
    )
}